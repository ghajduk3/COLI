...
class Preparator(object):
    ...
    def prepare_dataset_1(input_file_path:str):
        ...
        :return:
        """
        labels,data = utils.read_file_contents(input_file_path,':')
        ...
        utils.write_to_file(data,output_path)
    ...
    def prepare_dataset_2(input_file_path:str):
        ...
        :return:
        """
        data = utils.read_file_contents_pd(input_file_path)
        data['text'] = data.apply(lambda row: utils.transform_text(row.text, row.hate_speech_idx), axis=1)
        ...
        utils.write_to_file(comments, output_path)
        logger.info("Dataset_2 has been prepared")

    @staticmethod
    def prepare_dataset_3(input_file_path:str):
        """
        Reads the data from dataset_6. Converts it into format Text,Label(1-Hate,0-Non hate) and writes the contents to file.
        :param input_file_path:
        :return:
        """
        data = utils.read_file_contents_pd(input_file_path)
        data['text'] = data.apply(lambda row: utils.transform_dataset_2_3(row.text, row.hate_speech_idx), axis=1)
        comments = [item for sublist in data['text'].tolist() for item in sublist]
        comments.insert(0, ['Text', 'Label'])
    ...
    def prepare_dataset_4(input_file_path:str):
        ...
        utils.write_to_file(data['comment'].tolist(), output_path)
        logger.info("Dataset_4 has been prepared")

    @staticmethod
    def prepare_dataset_5(input_file_path:str):
        labels, data = utils.read_file_contents(os.path.join(rootpath.detect(),'source_data','dataset_5','annotations_metadata.csv'), ',')
        comments = []
        for row in data:
            file, user, forum, cont, label = row
            text = utils.read_file_contents_txt(os.path.join(rootpath.detect(),'source_data','dataset_5','all_files',file + '.txt'))
